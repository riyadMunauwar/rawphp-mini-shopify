                ...
                <!-- Content Start -->
                    <section class="bg-white p-5">
                        ...
                        <?php if(has('error', QUERY))  { ?>
                            <div class="py-2 mb-2 px-4 text-sm text-white bg-red-400"><?php echo get('error') ?? '' ?></div>
                        <?php } ?>

                        <?php if(has('success', QUERY))  { ?>
                            <div class="py-2  mb-2 px-4 text-sm text-white bg-green-400"><?php echo get('success') ?? '' ?></div>
                        <?php } ?>


                        <div>
                            <h1 class="text-lg font-semibold mb-8">Create Page</h1>
                            <form action="<?php route('store/update-page') ?>" method="POST" >
                                <input type="hidden" name="page-id" value="<?php echo $data['page']['id'] ?? '' ?>">
                                <div>
                                    <label class="block" for="name">Name</label>
                                    <input name="name" class="mt-2 border w-full py-2 px-4 focus:outline-0 focus:border-gray-900" id="name" type="text" value="<?php echo $data['page']['name'] ?? '' ?>" >
                                </div>
                                <div class="mt-5">
                                    <label class="block" for="title">Title</label>
                                    <input name="title" class="mt-2 border w-full py-2 px-4 focus:outline-0 focus:border-gray-900" id="title" type="text" value="<?php echo $data['page']['title'] ?? '' ?>">
                                </div>
                                <div class="mt-8">
                                    <textarea name="content" id="editor" cols="30" rows="10"><?php echo $data['page']['content'] ?? '' ?></textarea>                                 
                                </div>
