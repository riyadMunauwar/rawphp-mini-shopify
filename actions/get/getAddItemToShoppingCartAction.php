<?php 
    ...
    try
    {
        ...
        if(!$IfHasGetAll ){
            if($isAlreadyExists){
                return redirect("product-detail?p=$productID&error=This product is already exist.");
            }
        }
        ...
        if($isAlreadyExists){
            ...
            if($size){
               ...
               if(IsAreadyExist($isAlreadyExists, $size, 'size')){
                    return redirect("product-detail?p=$productID&error=This product already exist in the cart.");
               }
        ...
        if($stockQuantity && $stockQuantity >= 1 ){
            ...
            view('messagePage', ['success' => '', 'message' => 'Add to cart successfully !']);
            return;
